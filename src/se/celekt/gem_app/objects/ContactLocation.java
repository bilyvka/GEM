...
/**
 * Specialization of {@link android.location.Location} that also contains a flag that checks if the location has moved
 * <p> 
 ...
 * @version 1.0 
 */
public class ContactLocation extends Location implements Serializable{

	private static final long serialVersionUID = 5519051614009909415L;
	...
	public ContactLocation(String providerName){
	    ...
	    setAccuracy((float)Float.POSITIVE_INFINITY);

	}

    public ContactLocation(String providerName,double lat,double lon, float accuracy){
        super(providerName);
        hasMoved = false;
        setLatitude(lat);
        setLongitude(lon);
